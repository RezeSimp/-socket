...
def genera_richieste1(num,address,port):
    ...
    cognomi=["Falcone","Moraru","Pietra","Monte","Colombo"]
    ...
    if not data:
        ...
        print(f"La valutazione di {cognome} in {materia} è {valutazione}")
    s.close()
...
def genera_richieste2(num,address,port):
  ...
  #   esempio: pagella={"Cognome1":[("Matematica",8,1), ("Italiano",6,1), ("Inglese",9.5,3), ("Storia",8,2), ("Geografia",8,1)]}
    ...
    x=random.randint(0,4)
    cognome=cognomi[x]
    pagella={'cognome':cognome,cognome:[]}
    for i in range(len(materie)):
        pagella[cognome].append((materie[i],random.randint(1,10),random.randint(1,4)))
...
if __name__ == '__main__':
    ...
    #for i in range(NUM_WORKERS):
        #thread=threading.Thread(target=genera_richieste2, args=(i,SERVER_ADDRESS, SERVER_PORT))
